...
def get_strain(disp, x_wind=51):
    ...
    disp = np.squeeze(disp.cpu().numpy())
    strain = gaussian_filter(disp, sigma=[24,3])
...
def flow_warp(x, flow12, pad='border', mode='bilinear'):
    ...
    x_base = torch.arange(0, W).repeat(B, H, 1)  # BHW
    y_base = torch.arange(0, H).repeat(B, W, 1).transpose(1, 2)  # BHW
    base_grid = torch.stack([x_base, y_base], 1).type_as(x)  # B2HW
    # if x.is_cuda:
    #     base_grid = base_grid.to(torch.device("cuda:1") if torch.cuda.is_available() else torch.device("cpu"))

    # v_grid = norm_grid(base_grid.type_as(x) + flow12)  # BHW2
    v_grid = Variable(base_grid) + flow12
    _, _, H, W = v_grid.size()
    v_grid_norm = torch.zeros_like(v_grid)
    v_grid_norm[:, 0, :, :] = 2.0 * v_grid[:, 0, :, :] / (W - 1) - 1.0
    v_grid_norm[:, 1, :, :] = 2.0 * v_grid[:, 1, :, :] / (H - 1) - 1.0
    ...
    if 'align_corners' in inspect.getfullargspec(F.grid_sample).args:
        im1_recons = F.grid_sample(x, v_grid_norm, mode=mode, padding_mode=pad, align_corners=True)
    else:
        im1_recons = F.grid_sample(x, v_grid_norm, mode=mode, padding_mode=pad)
    return im1_recons
...
#     grid = torch.cat((xx ,yy) ,1).float().type_as(x)
...
#     if x.is_cuda:
#         grid = grid.to(torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu"))
#     vgrid = Variable(grid) + ddf
...
#     # 以下为增加部分（mask）
#     # mask = torch.autograd.Variable(torch.ones(x.size())).cuda(1)
